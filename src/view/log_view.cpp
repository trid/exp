...
void LogView::draw(SDL_Renderer *renderer) {
    ...
    if (dirty) {
        ...
        int height = TTF_FontHeight(view.getFont());
        int startHeight = 0;
        int surfaceHeight = height * messages.size();
        surface = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_TARGET, view.getWindowWidth(), surfaceHeight);
        ...
        for (string& str: messages) {
            SDL_Surface* renderedSurface = TTF_RenderText_Solid(view.getFont(), str.c_str(), textColor);
